define(function (require, exports) {
    ...
    var errmsg = {
        required: {
            msg: "必填选项!",
            test: function (obj) {
                return obj.val() ? obj.val().length > 0 : false;
            }
        },
        urlDomain: {
            msg: "域名不合法",
    ...
    function validateField(elem) {
        var name = null;
        var elem = $(elem);
        if (!elem.attr("data-validate")) return;
        var rules = elem.attr("data-validate").split(",");
        for (var i = 0,j = rules.length; i < j; i++){
            name = rules[i];
            if (errmsg[name] && typeof errmsg[name].msg == 'string' && !errmsg[name].test(elem)) {
                showErrors(elem, errmsg[name].msg);
                break;
            } else {
                hideErrors(elem);
            }
            if (name == "validOnlyCode") {
                var code = elem.val();
                var url = elem.attr("data-validUrl") + code;
                util.ajaxGet(url, "" , function(res){
    ...
    function showErrors(elem, errors) {
        ...
        //     "left":elem.position().left + elem.innerWidth() + 4,
        //     "z-index": 30
        // });        
    }
    ...
    exports.init = function(){
        $(".validate").die().live("blur", function () {
            validateField(this);
        });
    };

    exports.submitInit = function (pageid){
        var pageObj = util.paramJquery(pageid);
        pageObj.find(".validate").each(function(){
            validateField(this);
        });
        return pageObj.find(".err-border").size();
    }
